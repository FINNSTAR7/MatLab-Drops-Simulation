...
inputs = inputdlg({'Number of Samples',...
    'Probabilities', 'Parts'},...
    'User Input', [1 50; 1 50; 1 50], defs);
...
b.CData = c';
delta = freq(mode, x_vals, y_vals)*1.5375;
yLim = [0 delta]; set(gca, 'YLim', yLim);

y1 = freq(mode, x_vals, y_vals)*1.3625;
y2 = freq(mode, x_vals, y_vals)*1.1875;
y3 = freq(mode, x_vals, y_vals)*1.0125;
val = mode; [~, i] = max(x_vals >= val);
plot([val val], yLim, 'Color', cmap(i, :));
text(val, y1, sprintf([' Mode'...
    '\n x = ' num2str(val)...
...
str =['CI_{50%%}= ' char(177), num2str(0.6745*std_err, '%.3f')...
    '\nCI_{75%%}= ' char(177), num2str(1.1500*std_err, '%.3f')...
    '\nCI_{90%%}= ' char(177), num2str(1.6450*std_err, '%.3f')...
    '\nCI_{95%%}= ' char(177), num2str(1.9600*std_err, '%.3f')...
    '\nCI_{99%%}= ' char(177), num2str(2.5760*std_err, '%.3f')];
text(x_vals(end)*0.9875, yLim(2)*0.02275, sprintf(str),...
    'BackgroundColor', 'white', 'EdgeColor', 'black', 'Clipping', 'on',...
